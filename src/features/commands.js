...
async function generateSwitchTable(document, asJson) {
  let { sigHashes, collisions } = functionSignatureExtractor(
    document.getText()
  );

  if (collisions.length) {
    vscode.window.showErrorMessage(
      "Function sigHash collisions detected " + collisions.join(",")
    );
  }
...
async function generateEventSignatures(document, asJson) {
  ...
  if (asJson) {
    ...
    for (let hash in sigHashes) {
      ...
      content += `#define constant ${toUpperSnakeCase(
        name
      )}_SIGNATURE = 0x${hash}\n`;
    }
  }
...
function outputContentToSideEditor(content) {
  vscode.workspace
    .openTextDocument({ content, language: "huff" })
    .then((doc) =>
      vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside)
    );
}

// TODO: diff function ^ combine with above
function outputMdToSideEditor(content) {
  vscode.workspace
    .openTextDocument({ content, language: "md" })
    .then((doc) =>
      vscode.window.showTextDocument(doc, vscode.ViewColumn.Beside)
    );
}
