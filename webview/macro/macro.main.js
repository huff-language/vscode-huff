...
(function () {
  ...
  calldataInput.addEventListener("keypress", handleCalldataKeyPress);
  calldataInput.addEventListener("change", handleCalldataKeyPress);
  ...
  function renderStateSetter(id, key, value) {
    ...
    newSlot.innerHTML = `
            <input type="text" class="storage-key-input" placeholder="Slot Key" ${
              key && "value=" + key
            }>
            <input type="text" class="storage-value-input" placeholder="Value" ${
              value && "value=" + value
            }>
    ...
    newSlot
      .querySelector(".storage-key-input")
    ...
    newSlot
      .querySelector(".storage-value-input")
      .addEventListener("input", (e) => {
        // make setting state better
        const state = vscode.getState();
        state.stateValues[id] = {
  ...
  function prepareDebugSession() {
    ...
    ul.childNodes.forEach((node) =>
      node.childNodes.forEach((input) => argsArr.push(input.value))
    );
    ...
    const calldataChecked =
      document.querySelector(".calldata-checkbox").checked;
    ...
    const stateValues = state.stateValues
      ? Object.keys(state.stateValues).map((id) => state.stateValues[id])
      : [];
  ...
  function createStackInputs(event) {
    ...
    for (let i = 1; i <= macroIfo.takes; i++) {
      ...
      const state = vscode.getState();
      input.value =
        state.stackValues[selectedMacro] && state.stackValues[selectedMacro][i]
  ...
  function handleCalldataKeyPress(e) {
